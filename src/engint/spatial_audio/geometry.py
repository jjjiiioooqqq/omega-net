...
def ring_layout(count: int, radius_m: float = 2.0, height_m: float = 0.0) -> SpeakerLayout:
    """Evenly spaced horizontal ring centered on the origin, speaker 0 at +x."""
    if count < 2:
    ...
    speakers = [
        Speaker(
            name=f"ring_{i:02d}",
            position_m=(
                radius_m * float(np.cos(a)),
                radius_m * float(np.sin(a)),
                height_m,
            ),
        )
